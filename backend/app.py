...
from backend.controllers.distance import distance_bp
...
def create_app():
    ...
    @login_required
    def tutorial(): return render_template("tutorial/tutorial_etapa1.html")
    @app.route("/tutorial/etapa2", methods=["GET"])
    @login_required
    def tutorial_etapa2(): return render_template("tutorial/tutorial_etapa2.html")
    @app.route("/tutorial/etapa3", methods=["GET"])
    @login_required
    def tutorial_etapa3(): return render_template("tutorial/tutorial_etapa3.html")
    @app.route("/tutorial/etapa4", methods=["GET"])
    @login_required
    def tutorial_etapa4(): return render_template("tutorial/tutorial_etapa4.html")
    @app.route("/tutorial/etapa5", methods=["GET"])
    @login_required
    def tutorial_etapa5(): return render_template("tutorial/tutorial_etapa5.html")
    @app.route("/tutorial/etapa6", methods=["GET"])
    @login_required
    def tutorial_etapa6(): return render_template("tutorial/tutorial_etapa6.html")

    @app.route("/reward", methods=["GET"])
    @login_required
    def reward(): return render_template("reward.html")
    ...
    def home():
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT p.id,p.title,p.category,p.lat,p.lng,p.img_url,
                   p.hours,p.address,p.phone,p.price,
                   COALESCE(r.count_reviews,0),COALESCE(r.avg_rating,0)
            FROM places p
            LEFT JOIN (
              SELECT place_id,COUNT(*) AS count_reviews,
        ...
        raw_places=[{
          ...
          "img_url":row[5],"hours":row[6]or"",
          "address":row[7]or"","phone":row[8]or"",
          "price":row[9]or"","reviews":int(row[10]),
          "rating":float(row[11])
        } for row in rows]
        return render_template("menu/home.html",raw_places=raw_places)

    @app.route("/menu/puzzle", methods=["GET"])
    @login_required
    def puzzle(): return render_template("menu/puzzle.html")
    ...
    def locais():
        ...
        cur.execute("""
            SELECT
              ...
              p.img_url,
              p.features,
              COALESCE(r.count_reviews, 0) AS count_reviews,
        ...
        for row in rows:
            feats = row[6]
            try:
                # Se for string, tenta parsear JSON; senão usa como lista
                features = json.loads(feats) if isinstance(feats, str) else (feats or [])
        ...
        return render_template("menu/locais.html", raw_places=raw_places)

    @app.route("/menu/lojas", methods=["GET"])
    @login_required
    def lojas(): return render_template("menu/lojas.html")

    @app.route('/menu/detalhes/<int:place_id>',methods=['GET'])
    @login_required
    def detalhes(place_id):
        place=get_place_data(place_id)
        if not place:return render_template('404.html'),404
        hours=place.get('hours')or"";is_open=False
        try:
            o,c=hours.split('–');now=datetime.now().time()
            ot=datetime.strptime(o.strip(),"%H:%M").time()
            ct=datetime.strptime(c.strip(),"%H:%M").time()
            is_open=ot<=now<=ct
        except:pass
        return render_template('menu/detalhes.html',place=place,is_open=is_open)
    ...
    def perfil_comments():
        ...
        conn=conectar();cur=conn.cursor()
        cur.execute("""
            SELECT p.title,p.img_url,c.content,c.criado_em
            FROM comments c JOIN places p ON p.id=c.place_id
            WHERE c.user_id=%s ORDER BY c.criado_em DESC
        """,(user_id,))
        comments=[{
          'place_name':r[0],'place_img':r[1],
          'text':r[2],'date':r[3].strftime("%d/%m/%Y %H:%M")
